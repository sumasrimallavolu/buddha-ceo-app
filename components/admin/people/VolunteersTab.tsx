...
import { MoreVertical, Users, Clock, CheckCircle, XCircle, Mail } from 'lucide-react';
...
export function VolunteersTab({ canEdit }: VolunteersTabProps) {
  ...
  const handleDelete = async (id: string) => {
    ...
    setActionLoading(id);
    try {
      const response = await fetch(`/api/admin/volunteer-applications?id=${id}`, {
        method: 'DELETE',
      });
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-white/10 hover:bg-white/5">
                ...
                <TableHead className="text-slate-400 font-medium">Interest Area</TableHead>
              ...
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={canEdit ? 8 : 7} className="text-center py-12">
                    ...
                    <p className="text-slate-400">No volunteer applications found</p>
                  </TableCell>
                </TableRow>
              ) : (
                applications.map((application) => {
                  ...
                  return (
                    <TableRow key={application._id} className="border-white/10 hover:bg-white/5">
                      ...
                      <TableCell className="text-slate-400">{application.interestArea}</TableCell>
                      <TableCell>
