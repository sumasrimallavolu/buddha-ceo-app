...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check permission
    if (!hasPermission(session.user.role as any, 'view:teacher_applications')) {
    ...
    const applications = await TeacherApplication.find(filter).sort({ createdAt: -1 });
...
// PUT update teacher application status
